...
#----------------------------------------------------------------------#
    ...
    # -----------------------------------------------
    # Per-tile Min-Max normalization across all bands
    img_min = np.min(input_img, axis=(0, 1), keepdims=True)
    img_max = np.max(input_img, axis=(0, 1), keepdims=True)
    input_img = (input_img - img_min) / (img_max - img_min + 1e-8)  # Avoid division by zero
