...
def map_labels_to_colors(prediction):
    ...
    4: [0, 0, 128]      # Dark Blue for class 4
}
...
root_model_folder = os.path.join(root_image_folder, f'fcn_spectral_model&outcomes_{config_str}')
...
images = []
input_img_folder = os.path.join(root_image_folder, 'msi_rois/testing/map')

# Retrieve all image
img_files = [file for file in os.listdir(input_img_folder) if file.endswith(".tif")]
#----------------------------------------------------------------------#
# Load fcn model
fcn_model = load_model(os.path.join(root_model_folder,'fcn_save_best_model.hdf5'))
print("Model loaded")
#----------------------------------------------------------------------#
# Prediction
patch_size = 64
...
#----------------------------------------------------------------------#
    ...
    if input_img.shape[0] >= patch_size and input_img.shape[1] >= patch_size:
        emp = EMPatches()
        img_patches, indices = emp.extract_patches(input_img, patchsize=patch_size, overlap=0.1)
        ...
        img_patches_processed = fcn_model.predict(np.array(resized_patches))
        ...
        pred_image_file = os.path.splitext(img_files[i])[0] + '_pred.dat_map'

        # Define the path to the "prediction" folder
        prediction_folder = os.path.join(root_model_folder, 'fcn_prediction_map')
...
#----------------------------------------------------------------------#
        ...
        tif_file_directory = os.path.join(root_model_folder, 'fcn_prediction')
