...
#----------------------------------------------------------------------#
# Load fcn model
fcn_model = load_model(os.path.join(root_model_folder,'fcn_save_best_model.keras'))
print("Model loaded")
#----------------------------------------------------------------------#
# Prediction
patch_size = tile_size     # Updated patch size to 256 to match model input shape
...
#----------------------------------------------------------------------#
    ...
    if input_img.shape[0] >= patch_size and input_img.shape[1] >= patch_size:
        emp = EMPatches()
        img_patches, indices = emp.extract_patches(input_img, patchsize=patch_size, overlap=overlap_percentage)
        ...
        prediction_folder = os.path.join(root_model_folder, 'fcn_prediction_rois')
...
#----------------------------------------------------------------------#
        ...
        tif_file_directory = os.path.join(root_model_folder, 'fcn_prediction_rois')
