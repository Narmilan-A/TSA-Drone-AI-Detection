...
#----------------------------------------------------------------------#
# Only Rasterize Matching Files
def main():
    # Set file paths
    shapefile = "D:/tsa_model_training/seasons_for_labelling/nov-23/Site1/gt_labelling_TSA_Nov2023_Site1_P1_RGB_Orthomosaic_0.39cm.shp"
    input_image_folder = "D:/tsa_model_training/sensors_for_modelling/rgb/p1/rgb_rois/training"
    output_folder = "D:/tsa_model_training/sensors_for_modelling/rgb/p1/mask_rois/training"

    # Create the output folder if it doesn't exist
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    # Prefix to filter specific ROIs
    roi_prefix = "TSA_Nov2023_Site1_P1_RGB_Orthomosaic_"

    # Get a list of all .tif images
    image_list = glob.glob(os.path.join(input_image_folder, "*.tif"))

    # Loop and rasterize only matching images
    for image_path in image_list:
        image_name = os.path.basename(image_path)
        if image_name.startswith(roi_prefix):
            output_raster = os.path.join(output_folder, "mask_" + os.path.splitext(image_name)[0] + ".tif")
